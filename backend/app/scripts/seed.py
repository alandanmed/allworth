...
def seed() -> None:
    ...
    try:
        # --- Reference data: institutions + categories ---
        ...
        if db.query(Account).filter(Account.user_id == user.id).first():
            print("This user already has accounts — skipping. Delete rows manually to reseed.")
            return

        accounts = {
            "checking": Account(
                user_id=user.id,
                institution_id=institutions_by_name["Northwind Bank"].id,
            ...
            "credit": Account(
                user_id=user.id,
                institution_id=institutions_by_name["Horizon Credit Union"].id,
                name="Rewards Credit Card", type="credit_card",
                balance=Decimal("842.15"), last_four_digits="2210", sync_status="manual",
            ),
            "investment": Account(
                user_id=user.id,
                institution_id=institutions_by_name["Vantage Investments"].id,
        ...
        db.commit()
        print(f"Seeded {len(accounts)} accounts and {len(transactions)} transactions for {user.email}.")
